...
check-providers:
    cargo check -p omnifs-provider-github -p omnifs-provider-dns -p omnifs-provider-ipfs -p test-provider --target wasm32-wasip2
    cargo clippy -p omnifs-provider-github -p omnifs-provider-dns -p omnifs-provider-ipfs -p test-provider --target wasm32-wasip2 -- -D warnings
    cargo test -p omnifs-provider-github -p omnifs-provider-dns -p omnifs-provider-ipfs -p test-provider --target wasm32-wasip2 --no-run

build-providers:
    #!/usr/bin/env bash
    set -euo pipefail
    cargo build --target wasm32-wasip2 --release \
        -p omnifs-provider-github -p omnifs-provider-dns -p omnifs-provider-ipfs -p test-provider
