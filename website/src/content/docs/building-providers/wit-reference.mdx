...
import providerWit from '../../../../../wit/provider.wit?raw';

This page maps the SDK you write to the raw `omnifs:provider` WIT interface the host drives. You normally work through the SDK types; this is the contract underneath them. The annotated excerpts below are slices of `wit/provider.wit`; the [full interface is embedded verbatim](#the-full-interface-verbatim) at the end of this page, imported straight from the source file so it never drifts.
...
## The full interface, verbatim

The complete `omnifs:provider` interface, imported directly from `wit/provider.wit` at build time. This is the authoritative contract — the excerpts above are annotated slices of it.

<Code code={providerWit} lang="text" title="wit/provider.wit" />
