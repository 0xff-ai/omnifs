...
- **Frontend.** A protocol surface (FUSE or NFSv4 loopback) over the complete shared namespace, served by the separate slim `omnifs-thin` runner in `fuse` or `nfs` mode. It contains no engine runtime, Wasmtime, or provider bundle and attaches over the Omnifs VFS wire protocol. Public identity is filesystem (`fuse` or `nfs`) plus environment (`host`, `docker`, or `krunkit`) and an environment-owned location. The daemon never runs a frontend in-process.
...
- The CLI owns frontend launch and teardown through three environments: `host` (local process), `docker` (container), and `krunkit` (libkrun microVM on macOS). Docker and krunkit deliver FUSE only. Low-level delivery code may use `Driver::Local` and `FrontendDelivery::Local`; public commands, config, tables, and help use filesystem, environment, and location.
- Frontends are durable, independent access surfaces over all mounts. Strict `[[frontends]]` entries use `filesystem`, `environment`, and host-only `location`. When the list is absent or empty, platform defaults apply: Linux = host FUSE; macOS = host NFS plus Docker FUSE; other = host NFS. An explicit non-empty list replaces the defaults entirely. `omnifs frontend enable`, `disable`, `restart`, and `ls` own targeted lifecycle; top-level `down` stops every live frontend. `omnifs up` launches every effective entry unless `--no-frontend`; a daemon restart never replaces a live frontend process, and every environment reconnects through the wire protocol. The daemon always binds the fixed host socket `$OMNIFS_HOME/frontends/local.sock` (dir 0700, socket 0600; filesystem permissions are the auth). TCP (docker) and vsock (krunkit) listeners are bound on request and token-authenticated.
...
- `omnifs status` joins the desired plan, daemon attachments, mounts, provider pins, and runner fallback state into one plural `Inventory`. Human output is a workspace strip plus responsive resource tables; JSON/JSONL use the invocation-level `--output` envelope. A deliberately stopped, internally consistent workspace exits 0; actionable complete inventory exits 5; an expected but unreachable daemon exits 3.
...
- `omnifs shell` selects a frontend with `--environment` and host-only `--location`. Interactive ambiguity uses a picker; headless selection prefers Docker, then krunkit, then normalized host location.
- `omnifs setup` has no runtime choice and no host-location question: the location is not prompted for, it comes from the effective `[[frontends]]` plan or `OMNIFS_MOUNT_POINT`. Its Docker reachability row appears only when the effective frontend plan includes a Docker entry. `down`, `status`, and `shell` read the daemon and attachment inventory plus runner fallback state.
- `just dev` is the supported contributor runtime entrypoint: it runs `scripts/dev.ts` to build provider WASM into a content-addressed provider-store bundle, build the omnifs CLI natively, start a host-native daemon (`omnifs up --no-frontend`) with pinned dev mounts and credentials rendered into `~/.omnifs-dev`, launch the Docker FUSE frontend environment, and open a shell inside that frontend container at `/omnifs`. Uniform across OSes: the daemon is always host-native; a frontend location is how a contributor browses.
- The daemon runs on the host, not in a container: `omnifs status`, `omnifs down`, and the daemon log (`~/.omnifs-dev/cache/daemon.log`) all work directly, no `docker exec` needed. The frontend container is discoverable by its `ai.0xff.omnifs.home` label (`docker ps --filter label=ai.0xff.omnifs.home=~/.omnifs-dev`); reach its location with `docker exec -it -w /omnifs <name> /bin/sh` (it ships no zsh) and disable it with `omnifs frontend disable fuse --environment docker`.
...
- **Guest attach authority is not restored across daemon replacement yet.** Docker and krunkit runners retain their processes, but the replacement daemon does not yet restore the prior TCP/vsock address and token. `up` waits for reconnect and reports the still-running unattached frontend as degraded instead of replacing it; `omnifs frontend restart` is the explicit recovery until startup listener restoration lands.
