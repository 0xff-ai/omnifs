...
- `omnifs setup` has no runtime choice; its mount-point question and Docker reachability row apply only when the effective frontend plan includes a local or docker entry respectively. `down`/`status`/`shell` read the daemon and attachments from it and the runtime record.
