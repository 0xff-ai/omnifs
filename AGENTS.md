...
- `omnifs setup` defaults the runtime per OS and records it at `[system].runtime`: Linux and WSL default to native kernel FUSE (Docker is the opt-in second choice); macOS defaults to Docker (native loopback NFS is the experimental, read-only opt-in). The mount-point question is asked only when the chosen runtime is native. `omnifs up --runtime <docker|native>` overrides it for one launch without persisting; `down`/`status`/`shell` read the running backend from the daemon and launch record, never from `[system].runtime`.
- `just dev` is the supported contributor runtime entrypoint: it runs `scripts/dev.ts` to build provider WASM into a content-addressed provider-store bundle, build the omnifs CLI natively, start a host-native daemon (`omnifs up --runtime native`) with pinned dev mounts and credentials rendered into `~/.omnifs-dev`, attach the Docker-hosted FUSE frontend (`omnifs frontend up`) to it, and open a shell inside that frontend container at `/omnifs`. Uniform across OSes: the daemon is always host-native and the frontend container is always how a contributor browses the mount, even on Linux where the host-native mount is also directly visible.
