...
- Wire protocol v3 carries the connecting frontend's identity (kind + guest-side mount point, display-only). v2-or-lower clients are rejected outright with a named reason. Runtime is labeled by listener ownership at the daemon (UDS host, TCP docker, vsock libkrun); the guest never self-reports.
...
- The current lifecycle surface leaves guided onboarding composition to the CLI command layer: `up` starts only the daemon, and `frontend enable` starts one explicit runner. `down`, `status`, and `shell` read the daemon and observed attachment/runner inventory; no frontend desired state is persisted.
