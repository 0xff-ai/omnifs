...
- The CLI owns frontend launch and teardown through three environments: `host` (local process), `docker` (container), and `krunkit` (libkrun microVM on macOS). Docker and krunkit deliver FUSE only. Low-level delivery code may use `Driver::Local` and `FrontendDelivery::Local`; public commands, tables, and help use filesystem, environment, and location.
- Frontends are durable, independent access surfaces over all mounts. `omnifs frontend enable`, `disable`, and `restart` own runner lifecycle; `ls` reports observations only. `omnifs up` and its exact `apply` alias start or replace only the daemon, and `down` stops only the daemon. A daemon restart never replaces a live frontend process, and every environment reconnects through the wire protocol. `setup` may launch the platform defaults once as imperative actions: Linux = host FUSE; macOS = host NFS plus Docker FUSE. No frontend desired state is persisted. The daemon always binds the fixed host socket `$OMNIFS_HOME/frontends/local.sock` (dir 0700, socket 0600; filesystem permissions are the auth). TCP (docker) and vsock (krunkit) listeners are bound on request and token-authenticated.
...
- `omnifs status` joins daemon attachments, runner records, mounts, provider pins, and daemon state into one plural `Inventory`. Frontends are observed facts only; Inventory never synthesizes defaults or stopped desired rows. Human output is a workspace strip plus responsive resource tables; JSON/JSONL use the invocation-level `--output` envelope. A deliberately stopped, internally consistent workspace exits 0; actionable complete inventory exits 5; an expected but unreachable daemon exits 3.
...
- `omnifs setup` has no runtime choice and no host-location question. After starting the daemon through the same operation as `up`, it may imperatively enable the accepted platform defaults without writing frontend configuration. Its Docker reachability row appears only when the selected one-time actions include Docker. `down`, `status`, and `shell` read the daemon and observed attachment/runner inventory.
