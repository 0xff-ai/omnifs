...
- **Topology.** The `omnifsd` daemon loads provider WASM components (`wasm32-wasip2`) and drives them through the `omnifs:provider` WIT interface. The `omnifs` CLI owns host credentials and the daemon lifecycle and talks to `omnifsd` over an HTTP control API.
- **Mount frontends.** Two frontends serve the same projected tree: FUSE (Linux kernel, the container workflow) and a read-only NFSv4.0 loopback frontend (`crates/omnifs-nfs`, started via `omnifsd --frontend nfs`) that runs host-native, the motivating case being macOS where FUSE is not the right OS integration point. Do not reintroduce macFUSE, `diskutil`, or macOS-specific FUSE mount behavior; use the NFS path for non-Linux host integration. Details: `docs/design/nfsv4-loopback-mount.md`.
