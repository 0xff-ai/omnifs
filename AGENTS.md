...
- Frontends translate namespace answers into protocol state. They consume the narrow `omnifs_engine::namespace` surface, never `Tree`/`render`/`view` directly, and do not decide projection semantics. The daemon is a registry that serves several renderers over one shared namespace.
...
- **Frontend.** A protocol surface (FUSE or NFSv4 loopback) served by a separate slim runner binary (`omnifs-fuse` or `omnifs-nfs`). Runners contain no engine, Wasmtime, or provider bundle; they attach to the daemon's shared namespace over the Omnifs VFS wire protocol. "Frontend" is the protocol kind; "driver" (local, docker, krunkit) is the delivery mechanism chosen by the CLI. The daemon never runs a frontend in-process.
...
- The daemon is a pure namespace server and live frontend-attachment registry. It never builds, mounts, supervises, or unmounts a renderer and has no `--frontend` or named attach-socket flags. Every filesystem frontend is a separate slim runner binary (`omnifs-fuse` for FUSE, `omnifs-nfs` for NFSv4 loopback). Runners contain protocol mechanics only: no engine, no Wasmtime, no provider bundle, no daemon control plane. They attach over the Omnifs VFS wire protocol.
- The CLI owns frontend launch and teardown through three delivery drivers: `local` (host process), `docker` (container), `krunkit` (libkrun microVM on macOS). Docker and krunkit deliver FUSE only. "Frontend" names the protocol kind (fuse|nfs); "driver" names the delivery mechanism. "Backend" is internal to the CLI `FrontendBackend` trait and is never a user-facing term.
- Frontends are declared in strict `[[frontends]]` entries in config.toml (`kind`, `driver`, optional `mount_point` for local only). When the list is absent or empty, platform defaults apply: Linux = local FUSE; macOS = local NFS plus Docker FUSE; other = local NFS. An explicit non-empty list replaces the defaults entirely. `omnifs up` launches every effective entry unless `--no-frontend` (daemon only on every OS). The daemon always binds the fixed local socket `$OMNIFS_HOME/frontends/local.sock` (dir 0700, socket 0600; FS perms are the auth). TCP (docker) and vsock (krunkit) listeners are bound on request and token-authenticated.
- Wire protocol v3 carries the connecting frontend's identity (kind + guest-side mount point, display-only). v2-or-lower clients are rejected outright with a named reason. Delivery is labeled by listener ownership at the daemon (UDS local, TCP docker, vsock krunkit); the guest never self-reports.
- Local runners persist mount discovery state per mount under `cache/frontends/<kind>/<blake3-of-mount-path>` leaves; the NFS filehandle-identity table lives in the same leaf. Corrupt records degrade individually and never hide healthy siblings.
- `omnifs status` reports every live attachment (kind, delivery, mount point). `omnifs shell` prefers Docker, then krunkit, then a local mount.
- `omnifs setup` has no runtime choice and no mount-point question: the host mount point is not prompted for, it comes from the effective `[[frontends]]` plan or `OMNIFS_MOUNT_POINT`. Its Docker reachability row appears only when the effective frontend plan includes a docker entry. `down`/`status`/`shell` read the daemon and attachments from it and the runtime record.
- `just dev` is the supported contributor runtime entrypoint: it runs `scripts/dev.ts` to build provider WASM into a content-addressed provider-store bundle, build the omnifs CLI natively, start a host-native daemon (`omnifs up --no-frontend`) with pinned dev mounts and credentials rendered into `~/.omnifs-dev`, launch the Docker FUSE frontend via driver, and open a shell inside that frontend container at `/omnifs`. Uniform across OSes: the daemon is always host-native; the frontend container (or local mount) is how a contributor browses.
...
- `Dockerfile`'s `frontend-dev` stage is the contributor image path for `just dev` (the same target `just frontend-image` builds). There is no daemon-in-a-container image or stage: the daemon only ever runs host-native. The frontend image runs the slim `omnifs-fuse` binary (`crates/omnifs-fuse/src/bin/omnifs_fuse.rs`, a dedicated `[[bin]]` target of the `omnifs-fuse` crate that attaches through the Omnifs VFS wire protocol and serves a FUSE mount, with no engine, no Wasmtime, and no provider bundle), built by the `fuse-builder` Dockerfile stage with no provider-store build context. Release frontend image assembly uses `scripts/ci/build-frontend-image.sh`, injecting a prebuilt `omnifs-fuse` binary as the `omnifs-fuse-bin` build context; release CLI binaries (the separate full `omnifs` binary) embed the provider bundle and unpack it into `OMNIFS_HOME/providers`, and default their frontend image to the pinned `ghcr.io/0xff-ai/omnifs-frontend:<version>` tag, whereas a locally built binary defaults to the never-pulled `omnifs-frontend:dev` image.
