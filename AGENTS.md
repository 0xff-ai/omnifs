...
- **Config and schema must be wired.** Every config field in `#[omnifs_sdk::config]` must affect runtime behavior or be deleted. Defaults in the derived config schema, Rust config, and endpoint types must describe the same value.
