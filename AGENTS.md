...
- Provider installation and mount creation are separate. `provider add` retains artifacts only; `mount add` chooses an exact initial pin; `mount upgrade` is the sole post-creation repin command. Provider state is derived from exact reverse pins as `pinned`, `installed`, or `missing`, never from version or install recency. `omnifs up` never repins or rewrites a mount.
