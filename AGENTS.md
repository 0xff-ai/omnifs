...
- Frontends are durable, independent access surfaces over all mounts. `omnifs frontend enable`, `disable`, and `restart` own runner lifecycle; `ls` reports OS support and prerequisite readiness separately from instantiated observations. Host frontends permit multiple instances at distinct locations, while Docker and libkrun each permit one instance per workspace. `omnifs up` and its exact `apply` alias start or replace only the daemon, and `down` stops only the daemon. A daemon restart never replaces a live frontend process, and every runner reconnects through the wire protocol. `setup` offers every OS-supported frontend in a multi-select (the deterministic `frontend enable` default pre-checked; Docker never pre-checked) and launches the selection once as imperative actions. No frontend desired state is persisted. The daemon always binds the fixed host socket `$OMNIFS_HOME/frontends/local.sock` (dir 0700, socket 0600; filesystem permissions are the auth). Durable TCP/vsock targets live in `$OMNIFS_HOME/frontends/targets.json` (strict schema, atomic 0600 writes) and survive graceful daemon shutdown; unexpected listener exit removes only its exact target. TCP (docker) and vsock (libkrun) listeners are bound on request and token-authenticated.
- Wire protocol v7 carries the connecting frontend's identity (kind + guest-side mount point, display-only), engine-issued cache lifetimes, cacheable negative lookup answers, and the terminal `OfflineMiss` namespace error. Older clients are rejected outright with a named reason. Namespace identities are validated `Path` values and disconnects publish a root reset on the ordered event stream. Runtime is labeled by listener ownership at the daemon (UDS host, TCP docker, vsock libkrun); the guest never self-reports.
- Host runners persist frontend discovery state per location under `cache/frontends/<kind>/<blake3-of-location>` leaves; the NFS filehandle-identity table lives in the same leaf. Inventory also observes libkrun's workspace pidfile so a live detached VM remains selectable for reconnect, disable, and restart. Corrupt records degrade individually and never hide healthy siblings.
- `omnifs status` joins daemon attachments, runner observations, mounts with exact provider-pin health, and daemon state into one plural `Inventory`. Inventory stores only canonical frontend rows, never a second raw runner representation, and never synthesizes defaults or stopped desired rows. Human output is a workspace strip plus responsive resource tables; JSON/JSONL use the invocation-level `--output` envelope. A deliberately stopped, internally consistent workspace exits 0; actionable complete inventory exits 5; an expected but unreachable daemon exits 3.
...
- Guided onboarding belongs to `omnifs setup`, which composes provider configuration, `up`, and imperative user-selected frontend enables without persisting frontend desired state. `up` still starts only the daemon, and `frontend enable` still starts one explicit runner. `down`, `status`, and `shell` read the daemon and observed attachment/runner inventory.
...
- **A restored attach listener is part of daemon readiness.** Docker and libkrun runners retain their processes across daemon replacement; startup restores each durable TCP/vsock address from `$OMNIFS_HOME/frontends/targets.json` through `VfsServer` before publishing readiness. A restored listener failure aborts startup and leaves the target store intact for retry; `daemon.json` is process identity only and is removed on graceful shutdown.
