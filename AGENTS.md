...
- The daemon has one runtime mode: host-native. Docker delivers only the optional FUSE frontend (`omnifs frontend up`), attached to the host-native daemon over its TCP namespace listener; it is never how the daemon itself runs.
- Linux defaults to FUSE. macOS defaults to read-only NFSv4.0 loopback through `omnifs-nfs`.
- `omnifs setup` has no runtime choice to make; its mount-point question always applies, since the daemon is always host-native. `omnifs up` starts the host-native daemon; on macOS it additionally auto-starts the Docker-hosted FUSE frontend as the primary consumption surface (`--no-frontend` to skip it), with a truthful error naming Docker when it is absent. Linux `omnifs up` stays native-only; the frontend there is manual (`omnifs frontend up`). `down`/`status`/`shell` read the running daemon and any attached frontend from the daemon and runtime record.
- `just dev` is the supported contributor runtime entrypoint: it runs `scripts/dev.ts` to build provider WASM into a content-addressed provider-store bundle, build the omnifs CLI natively, start a host-native daemon (`omnifs up --no-frontend`) with pinned dev mounts and credentials rendered into `~/.omnifs-dev`, attach the Docker-hosted FUSE frontend (`omnifs frontend up`) to it, and open a shell inside that frontend container at `/omnifs`. Uniform across OSes: the daemon is always host-native and the frontend container is always how a contributor browses the mount, even on Linux where the host-native mount is also directly visible.
...
- `Dockerfile`'s `frontend-dev` stage is the contributor image path for `just dev` (the same target `just frontend-image` builds). There is no daemon-in-a-container image or stage: the daemon only ever runs host-native. The frontend image runs the slim `omnifs-fuse` binary (`crates/omnifs-fuse/src/bin/omnifs_fuse.rs`, a dedicated `[[bin]]` target of the `omnifs-fuse` crate that attaches a wire-backed namespace and serves a FUSE mount, with no engine, no Wasmtime, and no provider bundle), built by the `fuse-builder` Dockerfile stage, not the full `omnifs` CLI/daemon binary; unlike that binary's `builder` stage, `fuse-builder` needs no provider-store build context at all. Release frontend image assembly uses `scripts/ci/build-frontend-image.sh`, injecting a prebuilt `omnifs-fuse` binary as the `omnifs-fuse-bin` build context; release CLI binaries (the separate full `omnifs` binary) embed the provider bundle and unpack it into `OMNIFS_HOME/providers`, and default their frontend image to the pinned `ghcr.io/0xff-ai/omnifs-frontend:<version>` tag, whereas a locally built binary defaults to the never-pulled `omnifs-frontend:dev` image.
