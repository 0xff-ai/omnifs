...
- Wire protocol v6 carries the connecting frontend's identity (kind + guest-side mount point, display-only), engine-issued cache lifetimes, cacheable negative lookup answers, and the terminal `OfflineMiss` namespace error. v5-or-lower clients are rejected outright with a named reason. Namespace identities are validated `Path` values and disconnects publish a root reset on the ordered event stream. Runtime is labeled by listener ownership at the daemon (UDS host, TCP docker, vsock libkrun); the guest never self-reports.
...
- **A restored attach listener is part of daemon readiness.** Docker and libkrun runners retain their processes across daemon replacement; startup restores each durable TCP/vsock address and token from `$OMNIFS_HOME/frontends/targets.json` through `VfsServer` before publishing readiness. A restored listener failure aborts startup and leaves the target store intact for retry; `daemon.json` is process identity only and is removed on graceful shutdown.
