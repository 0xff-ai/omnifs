...
- **Three host caches, named by role.** **Object cache**: durable primary (`object` fjall keyspace, global, mount-prefixed keys); holds canonical upstream bytes keyed by logical object id; object-shaped providers only (a provider that emits no `canonical-store` effect self-selects out). **View cache**: derived, non-durable (`view` fjall keyspace, deleted and recreated on every startup); the rendered representations/fields/dirents shell tools read; recomputes from the object cache with no upstream refetch. **Blob cache**: large binary served by handle (+ archive trees), unchanged.
