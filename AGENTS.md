...
- The CLI owns OAuth/static-auth UX, profile config, the legacy mutation journal (`client/mutations.json`), metrics, daemon spawn, and resource authoring. It talks to the daemon only through typed local RPC. Legacy filesystem specs remain read-only migration input; normal lifecycle never launches from them.
...
- The transitional `fs create|attach|detach|restart|rm|shell|ls --name <id>` grammar now edits or reads daemon desired Attachments. Commands wait on typed revision or action progress by default. `fs shell` executes only the typed argv returned by `GetAttachmentAccess`. Old specs under `client/filesystems/specs` are scanned read-only and require explicit import.
...
- `omnifs status` still adapts desired Attachments and live sessions into the transitional filesystem view. `omnifs doctor` owns legacy and stray reporting and requires a cleanly stopped daemon, consent, and fresh exact identity proof before destructive remediation.
