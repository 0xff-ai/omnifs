...
- **A restored attach listener is part of daemon readiness.** Docker and krunkit runners retain their processes across daemon replacement; startup restores each durable TCP/vsock address and token through `VfsServer` before publishing readiness. A restored listener failure aborts startup and leaves the predecessor runtime record as stale diagnostic evidence.
