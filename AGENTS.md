...
- `omnifs setup` defaults the runtime to Docker on every OS and records it at `[system].runtime`; host-native is opt-in (loopback NFS on macOS, experimental; kernel FUSE on Linux/WSL). The mount-point question is asked only when the chosen runtime is native. `omnifs up --runtime <docker|native>` overrides it for one launch without persisting; `down`/`status`/`shell` read the running backend from the daemon and launch record, never from `[system].runtime`.
