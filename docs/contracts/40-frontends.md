...
The fail-closed lockdown check every runner's launch path runs immediately after start, killing the guest on violation, is part of the runner contract, not a Docker particular. Docker asserts no mounts and an env set of exactly `OMNIFS_ATTACH_ADDR` plus the image's own defaults. Libkrun asserts, against the live process's own argv (`ps`, since macOS has no `/proc`): no `virtio-net` device, exactly the expected device count, the two disk devices (root + seed), and the three vsock devices at their expected socket paths — plus a seed audit that the per-launch seed ISO's staging directory carries exactly its expected `KEY=VALUE` keys before it is burned. Both runners fail the launch (never report success) on any violation.
