...
Inventory also observes libkrun's workspace pidfile when no live daemon attachment exists. A live VM is reported as a running FUSE/libkrun frontend at its guest-owned `/omnifs` location, so enable can wait for reconnect and disable or restart can select the exact runner. A stale or unreadable pidfile degrades only that runner observation.
...
The fail-closed lockdown check every runner owns is part of the runner contract, not a Docker detail. Docker asserts no mounts and an env set of exactly `OMNIFS_ATTACH_ADDR` plus the image's own defaults. Libkrun enforces the device and feature set inside the typed helper before it publishes its pidfile, audits the seed's exact `KEY=VALUE` key set before burning it, and proves the guest-visible result in the live conformance lane: only loopback networking and no `tsi_hijack` kernel argument. Both runners fail launch before reporting success on any violation.
