...
**Ownership.** `async_singleflight::Group` owns exact-key OAuth refresh dedupe in `omnifs-auth`. `omnifs-nfs::delayed::PendingListings` owns exact-node listing slots, detached completion, and the inline wait budget for proactive `DELAY` signalling. `omnifs_engine::coalesce::Coalesce` owns covering-key namespace coalescing for provider ops. The engine namespace computes truth and owns cache; it does not know about `NFS4ERR_DELAY` or wait budgets. Reactive `Status::from(&NsError)` maps transient upstream errors without background continuation. FUSE owns its own blocking tolerance; it has no `DELAY` equivalent.
