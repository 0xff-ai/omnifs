...
Frontends split into two classes. **Native** frontends are served in-process by the daemon: FUSE on Linux, NFSv4 loopback on macOS. They have no driver, no attach transport, and no seam; the daemon owns them directly. **Virtualized** frontends run the same slim `omnifs-fuse --mount-point <path>` binary (`crates/omnifs-fuse/src/bin/omnifs_fuse.rs`, a dedicated `[[bin]]` target of the `omnifs-fuse` crate — no engine, no Wasmtime, no provider bundle) and wire protocol inside a guest, attached to the host-native daemon's namespace listener over an attach transport (TCP for Docker, vsock for krunkit). Which class serves a given OS is a daemon/CLI concern, not a frontend-crate one.
