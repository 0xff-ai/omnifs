...
The separate `omnifs-fuse --attach <socket> --mount-point <path>` binary (`crates/omnifs-fuse/src/bin/omnifs_fuse.rs`, a dedicated `[[bin]]` target of the `omnifs-fuse` crate, not a CLI subcommand) attaches a wire-backed namespace and serves a FUSE mount over it, blocking until unmount; SIGTERM/SIGINT unmount cleanly. It runs no provider and needs no engine, so it ships without Wasmtime, the provider bundle, or the daemon's control-plane surface — this is the whole content of the Docker frontend image's `ENTRYPOINT` and the krunkit guest's `omnifs-frontend.service`. With `--attach` absent, it attaches over TCP or vsock instead, reading the target from `OMNIFS_ATTACH_ADDR`/`OMNIFS_ATTACH_TOKEN` (the Docker-hosted frontend dials TCP; the krunkit guest dials `vsock:<port>`; neither can share a host Unix socket into its guest). The wire client reconnects with backoff and exposes `AttachEvent::Reattached` when a reconnect lands on a different daemon instance (stale node ids); FUSE's inode table is in-memory per-mount state a restart cannot invalidate out from under it, so the runner only logs the event. `AttachTarget` resolution, the readiness beacon, and the wire client live in `omnifs-namespace-wire` (`crates/omnifs-namespace-wire/src/client.rs`, `crates/omnifs-namespace-wire/src/beacon.rs`) so the FUSE-only binary needs no `omnifs-daemon` (and no Wasmtime dependency chain). No shipped binary serves NFS out of process; that leg exists as `omnifs-itest`'s `wire-test-frontend` test double (`crates/omnifs-itest/src/bin/wire_test_frontend.rs`), which the wire reattach/perf/parity live-acceptance lanes spawn over the same wire client and attach resolution.
...
- `crates/omnifs-namespace-wire/src/beacon.rs`
