...
The CLI materializes `HEAD` under cache storage and starts the daemon with that immutable snapshot plus its exact revision. The daemon never invokes Git or chooses desired state. If a healthy daemon already records `HEAD`, `up` leaves it running and only converges the frontend plan. Otherwise it stops only the daemon, starts the new revision, waits for readiness, and then advances `refs/omnifs/applied`; a failed start never advances the ref. Existing frontend processes are never replaced merely because the daemon restarted, and `up` may launch a configured frontend only after proving its runner is absent. Host frontends reconnect through the fixed local socket. Docker and krunkit processes are preserved, but daemon replacement does not yet restore their prior TCP/vsock address and token, so `up` reports them degraded when they cannot reconnect; `omnifs frontend restart` is the explicit recovery. `down` remains the explicit full stop.
...
`omnifs setup` has no runtime stage: the daemon is always host-native. It never asks for a host location; that comes from the effective `[[frontends]]` plan or `OMNIFS_MOUNT_POINT`, and its Docker reachability row appears only when the effective plan includes Docker. Frontend configuration is strict and durable:
...
Reset receipts retain the applied `verdict` and `rows` fields and add `dry_run` plus the typed `plan`. A dry run emits one receipt with `dry_run: true`, the planned rows, and no applied rows; an applied reset emits the same shape with `dry_run: false` and settled rows.
