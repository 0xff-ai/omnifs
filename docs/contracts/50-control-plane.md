...
Frontend defaults are never synthesized or persisted. Users start the daemon with `omnifs up` and enable each desired frontend explicitly with `omnifs frontend enable`; Linux commonly uses host FUSE, while macOS commonly uses host NFS plus optional Docker FUSE.
