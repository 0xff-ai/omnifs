...
The CLI resolves which endpoint to dial in one order: `OMNIFS_DAEMON_ADDR` when set (TCP, bearer token from `OMNIFS_CONTROL_TOKEN`), else the workspace's `daemon.json` (unix socket), else the fixed `$OMNIFS_HOME/control.sock` when present, else the daemon is not running (exit 3, no blind port dialing). An unreadable runtime record is treated as stale metadata and uses only that fixed socket fallback; if it is absent or refuses connections, the CLI may remove the corrupt record and reports the daemon as unavailable. It asserts the `instance_id` echoed by `/v1/status` against the record's, so a record overwritten by a restart mid-command is caught. Because the CLI only ever dials an endpoint from its own workspace's record or fixed socket, a daemon owned by a different `OMNIFS_HOME` is structurally unaddressable.
...
Every top-level error carries a stable, machine-stable slug derived from its exit class, not its wording (`generic-failure`, `usage`, `daemon-unavailable`, `auth-required`, `degraded`, `canceled`). The human error block shows it dim as a trailing `(id: <slug>)`; a `--json` command that fails before its final document emits the same slug as the `id` field of a single JSON error document on stdout.

`--json` is available on the state-reporting commands (`status`, `mount ls`, `provider ls`, `version`, `doctor`) and on the mutating and lifecycle commands (`up`, `down`, `reset`, `mount add`). The contract is: exactly one JSON document on stdout, all prose and narration on stderr, receipt facts are structured rather than encoded as preformatted summaries, and a machine-visible `fix` appears whenever recovery has a concrete next command. Error documents still carry a human-readable `message` alongside their stable `id`. Receipts are typed `#[derive(Serialize)]` structs that reuse the status view types, never hand-rolled `json!`. The `up` receipt carries the daemon, its mounts and frontends, and a verdict, and exits 5 when the verdict is degraded. A `--json` failure that has already emitted its receipt (a failed teardown row) returns the non-zero exit code without emitting a second error document.
...
`down` and the daemon phase of `reset` treat shutdown as complete only after
the acknowledged shutdown request's control surface becomes unavailable. The
CLI polls that surface with a bounded timeout and reports a failed teardown
row if the daemon remains reachable, so a successful `DaemonStopped` outcome
always means a subsequent status probe can observe `not_running`.

`reset --json` receipts retain the applied `verdict` and `rows` fields and add
`dry_run` plus the typed `plan`. A dry run emits one receipt with
`dry_run: true`, the planned rows, and no applied rows; an applied reset emits
the same shape with `dry_run: false` and settled rows.
