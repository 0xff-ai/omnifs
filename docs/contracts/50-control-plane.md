...
`/v1/ready` reports ready only after the immutable mount revision loads completely and every fixed, requested, or restored namespace listener has bound and remains supervised. Listener readiness does not require a frontend to be attached. `DaemonStatus.frontends` is the authoritative location set; API 8 has no singular daemon mount-point projection or failed-mount collection.
