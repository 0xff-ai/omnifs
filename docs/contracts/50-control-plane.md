...
There is one daemon runtime: host-native. `omnifs up` always spawns a host-native child process; there is no Docker daemon runtime and no `--runtime`/`[system].runtime` choice to make. Host-native frontend defaults are FUSE on Linux and NFSv4 loopback on non-Linux.
...
The daemon is a multi-frontend registry: it builds one renderer per requested frontend over a single shared namespace. The `--frontend <kind>=<mount_point>` daemon flag (repeatable, `fuse` or `nfs`) serves an explicit set; absent, the daemon serves the single platform default at the resolved mount point. This is a daemon surface only, not yet CLI-exposed; the launcher passes no `--frontend` flag, so its behavior is unchanged. `DaemonStatus.frontends` reports the served set. Linux can serve FUSE and NFS concurrently; macOS is NFS-only.

`omnifs setup` has no runtime stage: its mount-point question always applies, since the daemon is always host-native. `omnifs up` starts the host-native daemon; on macOS it additionally auto-starts the Docker-hosted FUSE frontend as the primary consumption surface, surfacing a truthful error naming Docker when it is unreachable without failing the native mount that already succeeded. `--no-frontend` skips that auto-start. Linux `omnifs up` stays native-only; the frontend there is manual (`omnifs frontend up`).
...
The daemon can serve its shared namespace over a Unix socket so a renderer runs in a different process than the projection owner. The `--attach-socket <name>` daemon flag (repeatable, a bare `[a-z0-9-]+` label) binds `$OMNIFS_HOME/frontends/<name>.sock` and serves the same `TreeNamespace` the in-process frontends use, with the daemon's instance id. The `frontends/` dir is `0700` and each socket `0600` (auth is filesystem permissions, like the control socket); a stale socket is unlinked after a refused connect probe, and sockets are removed on graceful exit. The Omnifs VFS wire protocol uses length-delimited postcard framing from `omnifs-vfs-wire`, not an RPC framework. It transports the engine-owned `Namespace` surface and does not own projection semantics.
...
The separate `omnifs-fuse --attach <socket> --mount-point <path>` binary (`crates/omnifs-fuse/src/bin/omnifs_fuse.rs`, a dedicated `[[bin]]` target of the `omnifs-fuse` crate, not a CLI subcommand) attaches a VFS-wire-backed namespace and serves a FUSE mount over it, blocking until unmount; SIGTERM/SIGINT unmount cleanly. It runs no provider and needs no engine, so it ships without Wasmtime, the provider bundle, or the daemon's control-plane surface — this is the whole content of the Docker frontend image's `ENTRYPOINT` and the krunkit guest's `omnifs-frontend.service`. With `--attach` absent, it attaches over TCP or vsock instead, reading the target from `OMNIFS_ATTACH_ADDR`/`OMNIFS_ATTACH_TOKEN` (the Docker-hosted frontend dials TCP; the krunkit guest dials `vsock:<port>`; neither can share a host Unix socket into its guest). The VFS wire client reconnects with backoff and exposes `AttachEvent::Reattached` when a reconnect lands on a different daemon instance (stale node ids); FUSE's inode table is in-memory per-mount state a restart cannot invalidate out from under it, so the runner only logs the event. `AttachTarget` resolution, the readiness beacon, and the client live in `omnifs-vfs-wire` (`crates/omnifs-vfs-wire/src/client.rs`, `crates/omnifs-vfs-wire/src/beacon.rs`) so the FUSE-only binary needs no `omnifs-daemon` (and no Wasmtime dependency chain). No shipped binary serves NFS out of process; that leg exists as `omnifs-itest`'s `wire-test-frontend` test double (`crates/omnifs-itest/src/bin/wire_test_frontend.rs`), which the VFS wire reattach/perf/parity live-acceptance lanes spawn over the same client and attach resolution.

`omnifs frontend {up,down,status}` is the visible lifecycle for the optional Docker-hosted FUSE frontend, a separate credential-free container attached to a host-native daemon's TCP namespace listener; it is not a daemon runtime mode. `up` ensures a host-native daemon is running (launching one if none is; the daemon only ever runs host-native, so any running daemon qualifies), binds the TCP attach listener via `POST /v1/frontend/attach-target`, resolves the frontend image by build-channel provenance (`ghcr.io/0xff-ai/omnifs-frontend:<version>` for a release binary, `omnifs-frontend:dev` for a dev binary, `[system].frontend_image`/`OMNIFS_FRONTEND_IMAGE` override), and runs the container with no binds, no `OMNIFS_HOME`, no docker.sock, no SSH agent, and no published ports: only `OMNIFS_ATTACH_ADDR`/`OMNIFS_ATTACH_TOKEN` cross into it. A fail-closed check right after start asserts an empty `Mounts` array and an env set of exactly those two vars plus the image's own defaults, killing the container on violation. One frontend container runs per workspace: `omnifs-frontend` for the default home, `omnifs-frontend-<hash8(home)>` otherwise. `down` removes the container; the attach listener itself has no close route (`POST /v1/frontend/attach-target` only ever binds, idempotently), so it stays bound until the daemon restarts. `omnifs down` tears down a running frontend container first. The runtime record's `frontends` entries gain an optional `via` field (`"docker"` for this frontend, absent for a host-native one).
...
The Omnifs VFS wire protocol also serves over token-authenticated TCP because a containerized frontend cannot share the host Unix socket. Docker Desktop forwards `host.docker.internal` to host loopback, so `--attach-tcp <port>` (`0` = ephemeral) and the default `POST /v1/frontend/attach-target` request bind `127.0.0.1:<port>`. Native Linux maps that name to the default Docker bridge gateway instead, so `omnifs frontend up` discovers that gateway from Docker and asks the daemon to bind only that address; the daemon verifies that the address is assigned to `docker0`. The route always selects an ephemeral port, must not bind `0.0.0.0`, and must not use host networking. The per-instance attach token is 32 hex chars, generated once per daemon start the first time TCP attach is requested. The route is idempotent, returning the already-bound address and token on a repeat call rather than rebinding; `frontend up` rejects an existing listener on the wrong address and asks the operator to restart the daemon. The handshake protocol bumped 2 to 3 to carry the connecting frontend's identity (its kind and guest-side mount point, display-only for the host) alongside the token: a v2 (or lower) client (or a wrong/missing token on a TCP listener) is rejected outright with a named reason, no negotiation fallback. The daemon's frontend registry tracks every attached client live from an `AttachObserver` the handshake fires into, labeled with the delivery mechanism (`docker` for this TCP listener, `krunkit` for the vsock-proxy UDS listener below, `local` for a plain `--attach-socket` listener) the daemon assigned at bind time, never from anything the guest claims about itself; `GET /v1/status`'s `frontends` list reports these attached entries alongside native ones. A plain `frontends/<name>.sock` attach listener still never checks the token (filesystem permissions are its whole auth); the token-checking UDS listener bound by `POST /v1/frontend/attach-target/vsock` (`frontends/vsock-attach.sock`, the krunkit vsock-proxy path, where every guest dial reaches the socket as the same local peer) requires it, same as TCP.
...
- `crates/omnifs-itest/src/bin/wire_test_frontend.rs`
