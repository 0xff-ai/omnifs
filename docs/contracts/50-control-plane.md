...
`omnifs mount update <name>` changes an existing spec without replacing fields the invocation did not name. Auth, provider config, and limits use explicit set or clear switches; at least one change is required. The command observes the committed revision and exact source before any auth flow, then verifies both under the repository lock before committing. A stale observation fails without writing. Commit failure restores the exact source and Git index. A no-op keeps the current revision. Changing to an auth scheme with no usable credential completes auth before the candidate commit, while `--no-auth` removes only the mount reference and never deletes a credential.

`mount add` resolves and validates the complete spec, retains the exact provider pin, and commits the spec before an optional sign-in. A failed sign-in keeps that valid desired spec and gives the exact `mount reauth` command. Raw secret values on the command line fail before persistence.
...
`omnifs fs shell --name <id> [--shell <path>] [--] [<argv>...]` probes only the runtime stored in the spec. Docker enters the exact labeled container. Libkrun enters the exact helper and VM. Host verifies its mounted phase and reports the ordinary host path. A trailing command preserves argv boundaries; it conflicts with `--shell`.
...
Static and followed logs preserve the daemon log's bytes, line endings, and missing final newline. `--quiet` suppresses only Omnifs narration, not log bytes. Human errors show one best recovery command; only a daemon launch failure includes a bounded daemon-log tail.

Doctor reports Environment, Workspace, Mounts, Filesystems, and Daemon sections, then one verdict. It deduplicates repairs, shows the complete repair set, asks once, attempts independent repairs after a failure, and rescans before returning the final verdict. `--no-input` never repairs, including with `--yes`.

Inspector uses a strict mode ladder: interactive human terminals get ratatui; `--plain` or a non-terminal gets human lines; JSONL gets typed event lines; JSON is a usage error. Its source queue and per-frame drain are bounded. Pause freezes only the view while ingestion continues. The activity view keeps a timestamped operation stream, path tree, per-mount metrics, and selected stage waterfall; compact mode keeps the header and stream. One keymap owns dispatch, footer, and help. It provides errors-only, next-error, latency order, idle hiding, copy-path feedback, pause, step, and timestamp-based replay speed. Disconnect keeps stale data under a clear waiting banner. Terminal and panic-hook guards restore raw mode, cursor, alternate screen, and the previous process hook. After restoration, Inspector prints a session receipt with duration, events, errors, cache ratio, slowest operation, and record path.
