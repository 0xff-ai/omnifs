...
OAuth client ids in provider declarations are public application identifiers, not secrets. User access tokens, refresh tokens, and client secrets remain sensitive host-side values. `omnifs init` owns first-run OAuth mount generation; `omnifs auth login <mount>` owns repair and re-authentication.
