...
The live registry stores `VfsSession` records keyed by `ResourceName`. A session is observed transport state, not a configured resource. Reconnect overlap may add another connection only when the Attachment name, every spec field, and runtime instance match. A connection that reuses a name with conflicting identity is rejected.
...
The public identity is the Attachment resource's `ResourceName` plus its daemon-owned, fully resolved `AttachmentSpec`: protocol, runtime, location, and runtime assets. `attachment add` asks only for values the daemon cannot infer; the planner resolves the complete spec before apply. Every launcher receives the name and exact spec through named arguments. Transport never infers identity. Attachment commands start the daemon when needed. Explicit `down` asks the daemon to stop and drain Attachments before it exits while preserving desired rows.
...
The private `omnifs-libkrun` sibling is the only libkrun process owner. It loads the absolute packaged `libexec/omnifs/libkrun.1.dylib`, uses the packaged EFI firmware, and accepts one strict typed configuration from the daemon runtime driver. That shape fixes two raw block devices, 2 vCPUs, 2048 MiB, serial output, three vsock ports, no GPU, and no network. It has no generic device, library, firmware, feature, or socket policy surface. It never searches `PATH`, invokes an external launcher, or exposes a REST control API. Its strict record carries the Attachment name, full resolved spec, PID, and random runtime instance. Detached teardown requires an identity-matched control Ping and Shutdown and never signals a PID read from disk. Only launch rollback may kill the unreaped child handle it directly owns.

The resolved guest image is an immutable base artifact. Each launch copies it into `daemon-state/runtime/attachments/<name>/libkrun/root.raw`, restricts that copy to mode `0600`, and passes only the copy to libkrun; the base image is never opened read-write or mutated. `root.raw` and any temporary copy are launch-owned artifacts removed by rollback, stale replacement, restart, and deletion, while the base image and persistent SSH key survive.

The fail-closed lockdown check every guest runner owns is part of the runner contract, not a Docker detail. Docker asserts no binds and an env set containing only `OMNIFS_ATTACH_ADDR` plus image defaults; the exact Attachment name, runtime instance, protocol, runtime, and location arrive as flat command arguments. Libkrun audits the seed's exact key set, including `OMNIFS_ATTACHMENT_NAME` and `OMNIFS_RUNTIME_INSTANCE`, before burning it and proves the guest-visible result in the live conformance lane: only loopback networking and no `tsi_hijack` kernel argument. Both runners fail launch before reporting success on any violation.

The libkrun guest's ssh access is keyed, not passworded: launch generates a per-Attachment ed25519 keypair under its daemon-owned runtime leaf on first use and embeds the public half in the seed as `OMNIFS_SSH_PUBKEY`. The guest installs it into root's `authorized_keys` and starts the ssh socket only when the seed carries a key; an omitted key leaves ssh disabled for that launch.
...
The shared NFS filesystem entrypoint attaches through the Omnifs VFS wire protocol. Host delivery reaches it through hidden `omnifs run-fs --protocol nfs`. Runner records and the persistent filehandle table live under `daemon-state/runtime/attachments/<name>`. Restarting an active filesystem must reuse the recorded server address for that Attachment leaf, never silently bind a new port and skip remounting. Corrupt leaves degrade individually.
...
The `omnifs-mtab` state files under a per-Attachment leaf are filesystem discovery and teardown state. Mount records carry mount point, address, and pid; the host runner record carries the Attachment name, exact spec, random runtime instance, process group, and control socket. The NFS filehandle-identity table (`omnifs-nfs/src/persist.rs`) is protocol identity, so it stays in `omnifs-nfs` with the filehandles, stateids, and inode table. It lives in the same Attachment leaf alongside the mtab files and mirrors their write discipline, but its schema and IO are NFS-crate-owned. Records degrade individually; healthy siblings are never hidden.
