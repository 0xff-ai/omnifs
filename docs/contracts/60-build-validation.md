...
The krunkit driver's guest (`docs/contracts/40-frontends.md`'s designated macOS successor backend) ships as a bootable raw disk image, not a container: `scripts/guest-image/` holds an `mkosi` project (`mkosi/mkosi.conf` plus `mkosi/mkosi.extra/` for the systemd units and tmpfiles rules) that assembles a minimal Debian trixie arm64 EFI image (systemd-boot, fuse3, no cloud-init, no sshd). `just guest-image` (`scripts/guest-image/build.sh`) extracts the linux/arm64 `omnifs` binary from the same shared `builder` Dockerfile stage the frontend image uses, then runs `mkosi` inside a privileged container (`scripts/guest-image/builder.Dockerfile`, since mkosi needs Linux loop devices and `systemd-repart` that macOS lacks) to bake it in at `/usr/local/bin/omnifs`. It needs a provider-store bundle first (`just providers build`) for the same reason `frontend-image` does: the CLI build embeds one even though the guest frontend runner never executes a provider.

Attach parameters (`OMNIFS_ATTACH_ADDR`, `OMNIFS_ATTACH_TOKEN`) reach the guest through a per-launch seed ISO, not cloud-init: `scripts/guest-image/make-seed-iso.sh` builds an ISO9660+Joliet volume labeled `OMNIFS-SEED` on macOS with `hdiutil makehybrid`, and the guest's `omnifs-seed-mount.service` mounts it by label before `omnifs-frontend.service` sources it via `EnvironmentFile=`. A missing seed volume or config file fails both units loudly in the journal; neither hangs silently. `just guest-image-smoke` (`scripts/guest-image/smoke.sh`) boots the image under `krunkit` with a throwaway seed carrying an unreachable placeholder address, and checks the serial console log for the guest reaching `multi-user.target` and `omnifs-frontend.service` starting; a successful attach is out of scope until the krunkit driver (the CLI-side launcher and its fail-closed lockdown check) lands in a later phase. This is a local dev-only pipeline today: there is no CI lane or release publishing for the guest image yet, unlike the frontend image above.
