...
Online access uses freshness deadlines to decide when to revalidate. Cache-only access ignores those deadlines because durable facts remain facts after their online TTL. Complete non-live bodies use an observation key even when dynamic content has no version token; online reuse still requires a stable or versioned-dynamic key. Exact positive and definitive negative lookups answer normally. A complete listing proves an unknown child is absent; a partial listing returns its known entries without provider-dependent controls or continuation, while unknown children remain `OfflineMiss`. Missing bodies, deferred/live/ranged values without a complete stored body, and provider-dependent continuation return terminal `OfflineMiss`.
