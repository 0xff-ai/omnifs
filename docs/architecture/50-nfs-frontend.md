...
NFS loopback mount startup and teardown are frontend delivery concerns. The CLI launches `omnifs-thin nfs` (local, or via a driver); the daemon serves the projected tree over the fixed local socket (and optional TCP/vsock listeners) and reports readiness through the control plane. The runner attaches and serves until unmount.
