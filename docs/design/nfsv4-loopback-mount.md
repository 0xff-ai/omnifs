...
Status: accepted
Scope: `crates/omnifs-nfs`, CLI NFS mount dispatch, host/frontend cache policy
...
The CLI starts this frontend through `omnifs daemon nfs-mount`. CLI dispatch
loads the provider registry, starts provider timers, constructs one
...
The NFS crate owns NFS server and mount mechanics only. CLI dispatch owns
provider registry lifecycle and global provider shutdown. Mount state is written
as structured JSON under a private state directory and removed when the mount
exits. CLI status reads that state registry rather than reverse-engineering
