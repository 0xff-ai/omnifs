...
- the **object cache** is the durable primary (canonical upstream bytes, keyed by anchor; `object` fjall keyspace);
- the **view cache** is derived and non-durable (`view` fjall keyspace, deleted on every startup), recomputed from the object cache with no upstream refetch;
