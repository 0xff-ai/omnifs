...
- **Object faces** are the default for a provider concept with identity and replayable canonical bytes. `r.object::<O>(template, |o| ..)` (dir-shaped) or `r.file_object::<O>(template, |o| ..)` (file-shaped) binds an `Object` to a template; under it, `o.file(name).canonical::<F>()` declares the verbatim stored bytes, `.representation::<F>()` a whole-object render, `.derive(method)` a field leaf (eager, or `.lazy()`), `.object::<C>()` a child-object-backed file, and `o.dir(name).collection::<C>(method)` / `.choices(..)` / `.children(..)` / `.tree(method)` the child topology. An object earns canonical storage only when it has a canonical face; an object with only direct/blob/stream faces (Docker's `Container`) emits no object-cache entry.
