...
Every mount root projects a host-synthesized, read-only `AGENTS.md` that documents the provider for navigating agents, built once at `Runtime` construction from `provider-info` plus the static manifest (`crates/omnifs-host/src/agents_doc.rs`). It is injected at the shared `Namespace` seam (`crates/omnifs-host/src/namespace.rs`), so both frontends inherit it; it is never frontend-specific. Collision-first: a provider that projects its own `/AGENTS.md` always wins, and the synthetic leaf is materialized only when the provider yields not-found at the root. The advertised size is the exact rendered byte length and `read` returns the same held buffer, so `stat`/`wc -c`/`cat` agree. This is the "self-describing README leaf" of the agent-legibility direction, derived host-side from metadata the host already holds (no new provider authority).

The router also accumulates a serializable route table (`RouteDescriptor`/`RouteManifest`, `crates/omnifs-sdk/src/router/routes.rs`) as each verb registers, exposed through `Router::route_manifest`. Because routes exist only after `start`, the table rides the `initialize` return rather than the static metadata section: the `#[provider]` glue reads it post-`seal` and ships it on `provider-info.routes` (a flattened tree; object children carry their parent's index, see `route-descriptor` in `provider.wit`). The host surfaces it through `Runtime::provider_info`. `just providers-introspect` (run by `providers-build`) drives each built component's `initialize` through the host loader and writes the route table to `providers/<name>/omnifs.routes.json` so docs and the CLI can read a provider's surface without a live daemon (`crates/omnifs-host/src/bin/introspect_routes.rs`).
