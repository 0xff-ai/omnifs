...
## 2. Provider SDK: one route API, faces under file and dir

There is one provider route API. `file` and `dir` are the filesystem nouns; the behaviors under them are *faces*, and the host is agnostic to all of it (faces are purely an SDK concept that lowers to byte-level WIT effects). The route topology stays readable from `start()`: an `Object` impl supplies behavior, never hidden route registration (`providers/DESIGN.md`, `crates/omnifs-sdk/src/router/object.rs`).

- **Object faces** are the default for a provider concept with identity and replayable canonical bytes. `r.object::<O>(template, |o| ..)` (dir-shaped) or `r.file_object::<O>(template, |o| ..)` (file-shaped) binds an `Object` to a template; under it, `o.file(name).canonical::<F>()` declares the verbatim stored bytes, `.representation::<F>()` a whole-object render, `.derive(method)` a field leaf (eager, or `.lazy()`), `.object::<C>()` a child-object-backed file, and `o.dir(name).collection::<C>(method)` / `.choices(..)` / `.children(..)` / `.tree(method)` the child topology. An object earns canonical storage only when it has a canonical face. A tree of direct/blob/stream reads with no canonical bytes at all (Docker containers, db tables) is modeled with raw `r.dir`/`r.file` routes, not an object.
- **Direct, blob, stream faces** declare their own read contract and are not cached as canonical. `o.file(name).direct(method)` invokes the upstream on the read (query/operational state, DNS answers), `.blob(method)` keeps host-resident bytes that never cross the WIT boundary (a PR diff), `.stream(method)` serves ranged or live bytes through `open-file`. `Stability::Live` belongs to a stream face, never to a whole object: a canonical model is a finite snapshot and a Version cannot be honest for a moving target.
- **Raw `r.dir` / `r.file` / `r.treeref` handlers** remain the low-level escape hatch for routes not honestly anchored in object identity. They emit no canonical-store effects.

The `Object` trait is small (`crates/omnifs-sdk/src/object.rs`): `type Key: Key` (identity + route context), `type State`, `type Canonical: Format` (the canonical content type, an associated format rather than a method, so JSON is just one option), `load` (fetch from upstream, honoring `since` as a conditional validator), and `decode` (parse the verbatim stored bytes back into a value, re-run on every warm read). `load` and `decode` live on `Object`; there is no `serde` bound (a private transport DTO may live inside `decode`). `#[omnifs_sdk::object(kind = "..", key = ..)]` emits the whole impl and forwards `load` to a provider-written inherent `async fn load`; `#[path_captures]` emits the `impl Key`.

Identity is two-layer, and the layering is the security boundary: `LogicalId = (Object::kind(), normalized non-Facet captures)` is computed in the provider (`Key::anchor`); `ObjectId = (mount, LogicalId)` is formed by the host. Mount-prefixing every object/view/reverse/negative/tombstone key is what stops two GitHub mounts with different credentials from sharing private canonical bytes for the same `owner/repo/number`. The WIT carries `logical-id { kind, captures }`; the host resolves a path to an id by exact map lookup, never a prefix probe (`crates/omnifs-cache/src/object.rs`).

A route Facet (`Facet<T>`) is route context excluded from identity, so several paths can resolve to one cached object. arXiv's version selector (`@latest` / `vN`) and a category prefix are Facets: `/papers/{paper}/@latest`, `/papers/{paper}/vN`, and `/categories/{category}/papers/{paper}/vN` are one object. The canonical bytes are exactly the verbatim upstream response body (never `serde(Self)`); the canonical face (and any representation whose content type is the canonical's own) is served from those bytes without calling `render`. A canonical may be assembled from several upstream calls into one blob; the provider folds the component validators into one opaque Version.
...
There is one byte-level read operation from the host's point of view: no provider-to-host canonical-read callout, no host-side render op. `read-file(id, path, content-type, cached-canonical)` materializes whole-file `Deferred(Full)` content; ranged `Deferred(Ranged)` content (stream faces) uses `open-file` / `read-chunk` / `close-file`; inline projected bytes (eager derived leaves) are served from cache without calling the provider. When the requested leaf is the canonical face, the SDK serves the stored bytes verbatim and never calls `render`; a representation face reconstructs `Self` via `Object::decode` and renders, and a derived face decodes then runs the registered field function. This is why there is no canonical/representation coherence hole.
...
- One route API: `file`/`dir` nouns with object, direct, blob, stream, and tree faces; object faces earn canonical storage only with a canonical face; raw handlers stay the escape hatch; Live is always a ranged stream face (sections 2, 8).
