...
use crate::wit_protocol::{cached_cursor_from_wit, cached_cursor_to_wit, entry_meta_from_kind};
...
impl Runtime {
    ...
    pub async fn paginate_next(&self, path: &Path, trace: Option<TraceId>) -> NextPageOutcome {
        ...
        let Some(record) = self.cache_get(path, RecordKind::Dirents, None) else {
        ...
        let listing = match self
            .namespace()
            .list_children(path, None, Some(cached_cursor_to_wit(cursor)), trace)
            .await
        {
            Ok(wit_types::ListChildrenResult::Entries(listing)) => listing,
            Ok(wit_types::ListChildrenResult::Unchanged) => {
        ...
        dirents.next_cursor = listing.next_cursor.clone().map(cached_cursor_from_wit);
...
fn append_listing_entries(
    entries: &mut Vec<DirentRecord>,
    new_entries: &[wit_types::DirEntry],
) -> usize {
    let mut added = 0;
    for entry in new_entries {
        ...
        entries.push(DirentRecord {
            ...
            meta: entry_meta_from_kind(&entry.kind),
        });
        added += 1;
    }
    added
