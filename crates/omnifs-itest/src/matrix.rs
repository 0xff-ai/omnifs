...
pub enum Exec {
    ...
    /// `omnifs attachment shell itest-libkrun -- <cmd>` into a libkrun
    ...
    fn command(&self, script: &str) -> Command {
        ...
        match self {
            ...
            Exec::SshLibkrun {
                ...
                cmd.args(["attachment", "shell", "itest-libkrun", "--", "env"])
...
/// The Docker-hosted FUSE filesystem (the desired `itest-docker` Attachment): a separate,
...
/// The libkrun guest FUSE filesystem (the desired `itest-libkrun` Attachment),
/// a libkrun microVM on Apple Silicon macOS, reached over ssh-over-vsock via
/// the real `omnifs attachment shell itest-libkrun -- <cmd>` CLI path
