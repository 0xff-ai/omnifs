...
fn test_ns() -> TestNs {
    ...
    let ns = Arc::clone(&harness.namespace);
    TestNs {
        ns,
        runtime,
...
async fn mount_child(ns: &dyn Namespace, name: &str) -> omnifs_engine::LookupAnswer {
    let root = ns.lookup(Path::root(), "test").await.unwrap();
    ns.lookup(root.path, name).await.unwrap()
}
...
async fn root_enumerates_and_descends_to_message() {
    ...
    let mount_page = ns
        ...
        .await
        .unwrap();
    assert!(mount_page.entries.iter().any(|entry| entry.name == "hello"));
    ...
    let attrs = ns
        .getattr(message.path.clone())
        .await
        .expect("getattr message");
...
async fn full_read_slices_by_offset_and_learns_size() {
    ...
    let message = ns.lookup(hello.path.clone(), "message").await.unwrap();

    // "Hello, world!" sliced at offset 2, length 4 => "llo,".
    let answer = ns
        .read(message.path.clone(), 2, 4)
        .await
        .expect("read slice");
...
async fn one_byte_probe_read_learns_size_for_next_getattr() {
    ...
    let fresh = ns.lookup(hello.path.clone(), "fresh-full").await.unwrap();
    let before = ns
        .getattr(fresh.path.clone())
        .await
        .expect("getattr before probe");
    assert_eq!(before.size, 1, "dynamic file starts on the sentinel");
    let answer = ns
        .read(fresh.path.clone(), 0, 1)
        .await
        .expect("dynamic probe read");
...
async fn ranged_read_reuses_one_handle_and_reports_eof() {
    ...
    let eof = ns.read(ranged.path.clone(), 26, 8).await.expect("eof read");
...
async fn readdir_pages_the_feed_with_synthetic_controls() {
    ...
    let hello = mount_child(ns.as_ref(), "hello").await;
    let feed = ns.lookup(hello.path.clone(), "feed").await.unwrap();
    ...
    let page1 = ns
        .readdir(feed.path.clone(), cursor0, 0)
        .await
        .expect("feed page 1");
...
async fn readdir_budget_buffers_overflow() {
    ...
    loop {
        let page = ns
            .readdir(hello.path.clone(), cursor, 1)
            .await
...
async fn stream_face_reads_through_open_file() {
    ...
    let items = mount_child(ns.as_ref(), "items").await;
...
async fn invalidation_bumps_epoch_and_notifies_subscriber() {
    ...
    let _ = ns
        ...
        .expect("prime read");
    assert_eq!(
        ns.getattr(message.path.clone()).await.unwrap().size,
        13,
    ...
    match event {
        NsEvent::InvalidateSubtree { path } => {
            assert_eq!(
                path,
                message.path.clone(),
                "the message path is invalidated"
            );
        },
