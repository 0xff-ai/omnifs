...
/// Inputs to open an offline [`Host`]: durable cache only.
...
impl Host {
    pub fn open_online(open: HostOpen) -> Result<Self, HostError> {
    ...
    #[must_use]
    pub fn as_online(&self) -> Option<&HostOnline> {
        match self {
            Self::Online(host) => Some(host),
            Self::Offline(_) => None,
        }
    }

    #[must_use]
    pub fn as_offline(&self) -> Option<&HostOffline> {
        match self {
            Self::Offline(host) => Some(host),
            Self::Online(_) => None,
        }
    }
    ...
    pub fn caches(&self) -> &Arc<Caches> {
        match self {
            Self::Online(host) => &host.caches,
            Self::Offline(host) => &host.caches,
        }
    }
    ...
    pub fn clone_dir(&self) -> &Path {
        match self {
            Self::Online(host) => &host.clone_dir,
            Self::Offline(host) => &host.clone_dir,
        }
    }
...
impl HostOnline {
    ...
    pub fn provider_path_by_id(&self, id: &ProviderId) -> PathBuf {
        self.catalog.store().artifact_path(id)
    }
}

impl HostOffline {
    /// View over an already-open cache for offline validation (no second open).
    pub(crate) fn from_open_caches(
        caches: Arc<Caches>,
        cache_dir: PathBuf,
        clone_dir: PathBuf,
    ) -> Self {
