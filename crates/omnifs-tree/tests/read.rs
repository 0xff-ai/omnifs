...
async fn open_ranged_then_read_chunks() {
    ...
    // open_file reports the provider's real attrs: Exact(26), Mutable.
    assert_eq!(handle.attrs().size, FileSize::Exact(26));
    assert_eq!(handle.attrs().stability, Stability::Mutable);
...
/// logical id), (b) durably cacheable (Immutable rendered representation ->
...
/// into the view cache). The Mutable-unversioned scalar fields
...
/// A cold `Tree::read` of an object-indexed, Inline-rendered, Immutable
...
/// even though it is Immutable and would otherwise be durably cacheable. The
