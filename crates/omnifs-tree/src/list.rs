...
use omnifs_wit::provider::types::{self as wit_types, ErrorKind};
use tracing::warn;
...
impl Tree {
    ...
    async fn list_continuation(
        ...
        match result {
            ...
            wit_types::ListChildrenResult::Subtree(_) => Err(TreeError::internal(
    ...
    async fn list_via_provider(
        ...
        match result {
            Ok(wit_types::ListChildrenResult::Entries(listing)) => Ok(ListOutcome::Listing(
            ...
            Ok(wit_types::ListChildrenResult::Subtree(tref)) => {
                let dir = runtime
            ...
            Err(Error::ProviderError(error)) => {
                ...
                Err(Error::ProviderError(error).into())
            },
            Err(error) => {
                warn!(path = path.as_str(), error = %error, "list_children runtime error");
                Err(error.into())
            },
        }
    }
...
fn provider_entries(entries: &[wit_types::DirEntry]) -> Vec<Entry> {
    entries
        .iter()
        .filter(|e| !synthetic::is_reserved_provider_leaf(&e.name))
        .map(|e| Entry::provider(e.name.clone(), entry_meta_from_kind(&e.kind)))
        .collect()
}
