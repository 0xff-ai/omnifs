...
use crate::config::{AuthConfig, AuthConfigKind, OAuthMountConfig, StaticTokenConfig};
use arc_swap::ArcSwapOption;
use async_singleflight::Group;
use fs2::FileExt;
use omnifs_auth::{AuthError as OAuthError, OAuthClient, OAuthRequest, OAuthRequestConfig};
use omnifs_creds::{
    CredentialEntry, CredentialId, CredentialKind, CredentialStore, FileStore, KeyringStore,
    StoreError,
};
...
pub enum AuthError {
    ...
    ManifestRequired(AuthConfigKind),
    ...
    CredentialStoreRequired(AuthConfigKind),
        ...
        match config {
            ...
            AuthConfig::OAuth(config) => {
                let context = store_context
                    .ok_or(AuthError::CredentialStoreRequired(AuthConfigKind::OAuth))?;
...
pub fn oauth_request_from_config(
    config: Option<&OAuthMountConfig>,
    scheme: OauthScheme,
...
impl StaticTokenStrategy {
    fn from_manifest_config(
        ...
        let manifest = manifest.ok_or(AuthError::ManifestRequired(AuthConfigKind::StaticToken))?;
        let scheme = manifest
...
impl OAuth2PkceStrategy {
    fn from_manifest_config(
        ...
        let manifest = manifest.ok_or(AuthError::ManifestRequired(AuthConfigKind::OAuth))?;
        let scheme = manifest
...
impl From<SchemeResolveError> for AuthError {
    fn from(error: SchemeResolveError) -> Self {
        match error {
            SchemeResolveError::NotFound { kind, key } => match kind {
                "static-token" => Self::StaticSchemeNotFound(key),
                _ => Self::OAuthSchemeNotFound(key),
            },
            SchemeResolveError::Ambiguous { kind } => match kind {
                "static-token" => Self::AmbiguousStaticScheme,
                _ => Self::AmbiguousOAuthScheme,
            },
            SchemeResolveError::NoSchemes { kind } => match kind {
                "static-token" => Self::StaticSchemeNotFound("<default>".to_string()),
                _ => Self::OAuthSchemeNotFound("<default>".to_string()),
            },
        }
    }
}

fn read_static_credential(config: &StaticTokenConfig) -> Option<String> {
...
fn read_oauth_client_secret(config: &OAuthMountConfig) -> Result<Option<SecretString>, AuthError> {
