...
impl InspectorSink {
    ...
    pub fn spawn_socket_server(self: &Arc<Self>, rt: &Handle) -> Option<JoinHandle<()>> {
        ...
        // Bind synchronously through std then convert; the caller
        // (mount_blocking) is on the tokio main thread, so `rt.block_on`
        // here would nest runtimes and panic.
...
impl crate::runtime::cloner::CloneObserver for InspectorCloneObserver {
...
impl WitCalloutView<'_> {
    ...
    fn summary(&self) -> String {
        match self.0 {
            ...
            _ => "callout.unsupported".to_string(),
        }
    }
}
