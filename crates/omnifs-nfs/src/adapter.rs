...
impl ObjectKey {
    fn new(scope: u64, mount: impl Into<String>, path: impl Into<String>) -> Self {
        Self {
            scope,
            mount: mount.into(),
            path: path.into(),
...
impl Export {
    pub fn new(rt: Handle, registry: Arc<ProviderRegistry>) -> Self {
        ...
        if let Some(mount) = &root_mount {
            path_to_inode.insert(ObjectKey::new(ROOT_ID, mount, ""), ROOT_ID);
        }
        ...
        if let Some(mount) = &root_mount {
            path_to_inode.insert(ObjectKey::new(EXPORT_ROOT_ID, mount, ""), EXPORT_ROOT_ID);
        }
    ...
    fn drain_invalidations_for_mount(&self, mount_name: &str) {
        ...
        let stale_negative_lookups = self
            ...
            .filter_map(|entry| {
                ...
                (key.mount == mount_name
                    && frontend::path_matches_invalidations(&key.path, &paths, &prefixes))
                .then(|| key.clone())
            })
            .collect::<Vec<_>>();
        ...
        let stale_opens = self
            ...
            .filter_map(|entry| {
                ...
                (ranged.mount_name == mount_name
                    && frontend::path_matches_invalidations(&ranged.path, &paths, &prefixes))
                .then(|| *entry.key())
            })
            .collect::<Vec<_>>();
        ...
        let stale_backing_opens = self
            ...
            .filter_map(|entry| {
                ...
                (backing.mount_name == mount_name
                    && frontend::path_matches_invalidations(&backing.path, &paths, &prefixes))
                .then(|| *entry.key())
            })
            .collect::<Vec<_>>();
    ...
    fn probe_ranged_attrs(&self, runtime: &Arc<Runtime>, path: &str) -> Option<FileAttrsCache> {
        let opened = match self
            .rt
            .block_on(runtime.namespace().open_file(&frontend::host_path(path)))
        {
            Ok(opened) => opened,
            ...
            Err(error) => {
                tracing::warn!(path, error = %error, "NFS ranged attr probe failed");
                return None;
            },
        };
        ...
        if let Err(error) = attrs.validate() {
            tracing::warn!(path, error, "NFS ranged attr probe returned invalid attrs");
            let _ = runtime.call_close_file(opened.handle);
            return None;
        }
        if let Err(error) = runtime.call_close_file(opened.handle) {
            tracing::warn!(path, error = %error, "NFS ranged attr probe close failed");
        }
        Some(attrs)
    }
    ...
    fn provider_status(error: &ProviderError) -> Status {
        match frontend::provider_fs_error(error) {
    ...
    fn lookup_from_cached_dirents(
        ...
        let lookup = frontend::cached_dirent_lookup(&record, name)?;
        if matches!(lookup, LookupCacheHit::Negative) {
            return None;
        }
        let path = frontend::join_child_path(parent_path, name);
        ...
        match lookup {
            ...
            LookupCacheHit::Positive(mut meta) => {
                self.negative_lookups
                    .remove(&PathKey::new(mount_name, child_path));
    ...
    fn lookup_via_provider(
        ...
        let child_path = frontend::join_child_path(parent_path, name);
        match self.rt.block_on(runtime.namespace().lookup_child(
            &frontend::host_path(parent_path),
            name,
            None,
        )) {
            ...
            Ok(LookupOutcome::NotFound) => {
                if Self::expected_negative_probe(name) {
                    self.negative_lookups
                        .insert(PathKey::new(mount_name, &child_path), ());
                }
            ...
            Err(RuntimeError::ProviderError(error)) => {
                if frontend::provider_fs_error(&error) == ProviderFsError::NotFound
                    && Self::expected_negative_probe(name)
                {
                    self.negative_lookups
                        .insert(PathKey::new(mount_name, &child_path), ());
                    tracing::debug!(
    ...
    fn lookup_from_parent_subtree(
        ...
        if parent_path.is_empty() || Self::expected_negative_probe(name) {
            return None;
        }
    ...
    fn readdir_from_dirents(
        ...
        let entries = dirents
            ...
            .iter()
            .map(|entry| {
                let child_path = frontend::join_child_path(path, &entry.name);
                self.negative_lookups
                    .remove(&PathKey::new(mount_name, &child_path));
    ...
    fn readdir_backing(
        ...
        for entry in std::fs::read_dir(root).map_err(|_| Status::Io)? {
            ...
            let child_path = frontend::join_child_path(path, name);
    ...
    fn read_provider_file(
        ...
        match self.rt.block_on(runtime.namespace().read_file(
            ...
            Ok(result) => {
                let Some(resolved) = frontend::resolve_read_payload(runtime, path, result) else {
                    tracing::warn!(path, "NFS read payload could not be resolved");
                    return Err(Status::Io);
                };
            ...
            Ok(opened) => {
                let opened_attrs =
                    match frontend::opened_file_attrs(Some(projected_attrs), &opened.attrs) {
                        Ok(attrs) => attrs,
                        Err(error) => {
                            tracing::warn!(path, error, "open-file returned invalid attrs");
                            let _ = runtime.call_close_file(opened.handle);
                            return Err(Status::Io);
                        },
                    };
...
impl ReadOnlyExport for Export {
    ...
    fn lookup(&self, parent: u64, name: &str) -> StatusResult<u64> {
        if ComponentName::parse(name).is_err() {
            return Err(Status::Invalid);
        }
        ...
        if parent == ROOT_ID && name == NFS_EXPORT_NAME {
        ...
        if Self::expected_negative_probe(name)
            && self
                .negative_lookups
                .contains_key(&PathKey::new(&mount_name, &child_path))
        {
...
mod tests {
    ...
    fn empty_export() -> TestExport {
        ...
        let registry = ProviderRegistry::empty_for_test();
    ...
    fn insert_backing_file(export: &Export, id: u64, path: &str, backing: PathBuf, size: u64) {
        export
            .path_to_inode
            .insert(ObjectKey::new(ROOT_ID, "test", path), id);
        export.entries.insert(
