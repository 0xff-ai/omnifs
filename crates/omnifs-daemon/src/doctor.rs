...
use omnifs_api::{
    ...
    DoctorRepairOutcome, DoctorRepairState, DoctorSection, DoctorSeverity, MountHealth,
    RunDoctorReport,
};
use omnifs_core::ResourceName;
use omnifs_state::{DesiredFilesystem, FilesystemInstance};
...
impl Daemon {
    ...
    pub(crate) async fn run_doctor(&self) -> Result<RunDoctorReport> {
        ...
        findings.push(docker_finding);
        findings.push(probe_fuse());
        findings.push(self.probe_image(docker.as_ref()).await);
        findings.push(self.probe_network().await);

        findings.push(self.probe_credential_store().await?);
        findings.push(probe_ssh_agent());
        findings.push(self.probe_config());
        ...
        let desired = self.state.desired_filesystems().await?;
        let instances = self.state.filesystem_instances().await?;
        let live = self.live_filesystems();
        let mut filesystem_findings = self
            .filesystem_findings(&paths, docker.as_ref(), &desired, &instances, &live)
            .await?;
    ...
    async fn apply_runtime_repair(&self, repair: DoctorRepair) -> Result<RepairResult> {
        ...
        if self.filesystem_is_owned(&name).await? {
            return Ok(RepairResult::Skipped(
                "filesystem became desired since diagnosis".to_owned(),
            ));
        }
        match repair {
            DoctorRepair::StopHost {
                ...
                let runtime = paths.filesystem(&name);
    ...
    async fn filesystem_is_owned(&self, name: &ResourceName) -> Result<bool> {
        let desired = self.state.desired_filesystems().await?;
        let instances = self.state.filesystem_instances().await?;
        Ok(desired
            .iter()
    ...
    async fn filesystem_findings(
        ...
        for candidate in candidates {
            match candidate {
                ...
                Candidate::Docker(owned) => findings.push(docker_candidate_finding(owned, self)),
                Candidate::Host {
                    state_dir,
                    record,
                    confirmed,
                    ...
                    let (finding, remediation) = self
                        .host_candidate_finding(
                            paths, state_dir, record, confirmed, desired, instances, live,
                        )
                        .await?;
                ...
                Candidate::Libkrun {
                    ...
                    let (finding, remediation) = self
                        .libkrun_candidate_finding(
                            filesystem, state_dir, confirmed, desired, instances, live,
                        )
                        .await?;
                    if let Some(finding) = finding {
                        findings.push(finding);
    ...
    async fn host_candidate_finding(
        ...
        let owned = filesystem_name_owned(&name, desired, instances);
...
fn docker_candidate_finding(owned: OwnedFilesystemContainer, daemon: &Daemon) -> DoctorFinding {
    finding_with_target(
        DoctorSection::Filesystems,
        ...
        Probe::Attention(format!(
            ...
            owned.identity.id,
            daemon.control_status().health.overall_state()
        )),
    )
}
...
fn probe_ssh_agent() -> DoctorFinding {
    let message = match std::env::var_os("SSH_AUTH_SOCK") {
        Some(sock) if Path::new(&sock).exists() => {
            format!("{} (daemon environment)", Path::new(&sock).display())
        },
        Some(_) => "SSH_AUTH_SOCK set but socket not found (daemon environment)".to_owned(),
        None => "SSH_AUTH_SOCK unset; git callouts will fail (daemon environment)".to_owned(),
    };
    let probe = if message.contains("socket not found") || message.contains("unset") {
        Probe::Attention(message)
    } else {
        Probe::Positive(message)
    };
    finding(
        DoctorSection::Profile,
...
mod tests {
    use super::*;
    use omnifs_core::{FilesystemProtocol, FilesystemRuntime, FilesystemSpec};
    ...
    #[test]
    fn desired_and_observed_names_are_ineligible() {
        let desired = vec![DesiredFilesystem {
            definition: omnifs_api::FilesystemDefinition {
                name: name("desired"),
            ...
            version: omnifs_core::FilesystemVersion::from_digest([1; 32]),
            revision: omnifs_core::ResourceRevision::initial(),
        }];
        let observed = vec![FilesystemInstance::pending(name("observed"))];
        assert!(!filesystem_name_owned(&name("stray"), &desired, &observed));
        assert!(filesystem_name_owned(&name("desired"), &desired, &observed));
        assert!(filesystem_name_owned(
            &name("observed"),
            &desired,
            &observed
        ));
