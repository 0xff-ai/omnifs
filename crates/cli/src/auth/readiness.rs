...
use omnifs_host::config::EffectiveConfig;
...
impl AuthReadiness {
    pub(crate) fn from_config(config: &EffectiveConfig, store: &dyn CredentialStore) -> Self {
        let target = match CredentialTarget::from_effective_config(config) {
            Ok(target) => target,
...
mod tests {
    use super::*;
    use omnifs_creds::{CredentialId, MemoryStore};
    use omnifs_host::config::InstanceConfig;
    use secrecy::SecretString;
    use time::OffsetDateTime;

    fn effective_config(json: &str) -> EffectiveConfig {
        InstanceConfig::parse(json)
            .unwrap()
            .into_effective("github", None)
            .unwrap()
    }
