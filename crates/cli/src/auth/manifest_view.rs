...
impl<'a> AuthManifestView<'a> {
    ...
    pub(crate) fn oauth_scheme(&self, requested: Option<&str>) -> anyhow::Result<&'a OauthScheme> {
        ...
        manifest
            ...
            .map_err(map_oauth_resolve_error)
    }
...
fn map_oauth_resolve_error(error: SchemeResolveError) -> anyhow::Error {
    match error {
        SchemeResolveError::NotFound { key, .. } => {
            anyhow!("no OAuth scheme `{key}` in auth manifest")
        },
        SchemeResolveError::NoSchemes { .. } => {
            anyhow!("auth manifest declares no OAuth scheme")
        },
        SchemeResolveError::Ambiguous { .. } => {
            anyhow!("multiple OAuth schemes are declared; set auth.scheme in the mount config")
        },
    }
}
