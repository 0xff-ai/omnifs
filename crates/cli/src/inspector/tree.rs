...
impl MountTree {
    ...
    /// Mark this node as a subtree handoff target. Wired through once
    /// the host emits subtree events (currently dead on the wire).
    #[allow(dead_code)]
...
impl MountForest {
    // Needed by selection-sync; render path uses render_rows below.
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    pub fn get(&self, mount: &str) -> Option<&MountTree> {
...
pub struct RenderRow {
    pub depth: usize,
    pub name: String,
    // Carried for forthcoming keyboard navigation (jump from a tree row
    // to the most-recent op that touched the path).
    #[allow(dead_code)]
    pub path: String,
    pub mount: String,
    pub status: NodeStatus,
