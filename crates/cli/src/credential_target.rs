...
use anyhow::Context;
use omnifs_creds::{CredentialEntry, CredentialId, CredentialStore};
use omnifs_host::config::{AuthConfig, EffectiveConfig};
use omnifs_model::{AccountId, AuthSchemeId, ProviderId};
use thiserror::Error;
...
impl CredentialTarget {
    ...
    pub(crate) fn for_static_import(
        ...
        let provider_id = ProviderId::new(provider_id)
            .map_err(|error| CredentialTargetError::InvalidKey(error.to_string()))?;
    ...
    pub(crate) fn for_mount(config: &EffectiveConfig) -> Self {
    ...
    fn internal_from_parts(
        ...
        let provider_id = ProviderId::new(config.provider_id())
            .map_err(|error| CredentialTargetError::InvalidKey(error.to_string()))?;
        let account = account
            .or_else(|| auth.and_then(AuthConfig::account))
            .map(AccountId::new)
            .transpose()
            .map_err(|error| CredentialTargetError::InvalidKey(error.to_string()))?
            .unwrap_or_else(AccountId::default_account);
