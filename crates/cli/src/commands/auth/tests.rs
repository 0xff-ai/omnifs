...
use super::shared::{load_mount, oauth_request, read_auth_manifest};
use super::status::status;
use crate::catalog::ProviderCatalog;
use crate::paths::Paths;
use omnifs_creds::{CredentialId, CredentialStore, MemoryStore};
...
fn schemes_reads_manifest_from_provider() {
    ...
    let catalog = ProviderCatalog::new(&paths.mounts_dir, &paths.providers_dir);
    let loaded = read_auth_manifest(&catalog, &load_mount(&catalog, "github").unwrap().config)
        .unwrap()
        .unwrap();
...
fn oauth_request_uses_builtin_github_manifest_without_provider_file() {
    ...
    let catalog = ProviderCatalog::new(&paths.mounts_dir, &paths.providers_dir);
    let (_mount, request, target) = oauth_request(&catalog, "github", None, &[]).unwrap();
...
fn oauth_request_uses_configured_client_id_when_manifest_has_no_default() {
    ...
    let catalog = ProviderCatalog::new(&paths.mounts_dir, &paths.providers_dir);
    let (_mount, request, _target) = oauth_request(&catalog, "example", None, &[]).unwrap();
...
fn oauth_request_cli_scopes_override_config_scopes() {
    ...
    let catalog = ProviderCatalog::new(&paths.mounts_dir, &paths.providers_dir);
    let (_mount, request, _target) =
        oauth_request(&catalog, "example", None, &["repo".to_owned()]).unwrap();
...
fn oauth_request_uses_provider_metadata_id_for_credential_id() {
    ...
    let catalog = ProviderCatalog::new(&paths.mounts_dir, &paths.providers_dir);
    let (_mount, _request, target) = oauth_request(&catalog, "example", None, &[]).unwrap();
...
fn write_mount(paths: &Paths, name: &str, json: &str) {
    let mount = omnifs_model::MountName::new(name.to_owned()).unwrap();
