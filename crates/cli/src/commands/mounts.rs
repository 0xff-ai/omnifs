...
pub fn rm(
    ...
    let effective = catalog
        .into_effective_mount(config, false)
        .with_context(|| format!("resolve mount config for `{name}`"))?;
    let credential_target = CredentialTarget::for_mount(&effective);
...
fn missing_mount_error(paths: &Paths, catalog: &ProviderCatalog, name: &str) -> anyhow::Result<()> {
    let suggestion = closest_mount_name(catalog, name)?;
...
fn closest_mount_name(catalog: &ProviderCatalog, target: &str) -> anyhow::Result<Option<String>> {
    catalog.closest_mount_name(target)
}
