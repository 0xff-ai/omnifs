...
mod tests {
    ...
    fn ensure_creates_ancestor_chain() {
        ...
        tree.mark_in_flight("0xff-ai/omnifs/pulls/open", 1, 100);
        let org = tree.root.children.get("0xff-ai").expect("0xff-ai");
        let omnifs = org.children.get("omnifs").expect("omnifs");
        let prs = omnifs.children.get("pulls").expect("pulls");
        let open = prs.children.get("open").expect("open");
        assert_eq!(open.path, "0xff-ai/omnifs/pulls/open");
