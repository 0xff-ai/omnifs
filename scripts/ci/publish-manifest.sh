...
# verify both platforms are present. Used by the filesystem-image publish step
# in ci.yml.
#
# Usage: publish-manifest.sh <image-repo> <tag> <digests-dir> <out-prefix>
#
# <image-repo> is the registry-qualified repo with no tag (e.g.
# ghcr.io/0xff-ai/omnifs-filesystem). <digests-dir> holds one file per platform,
# named by its sha256 digest (hex, no prefix), as `actions/download-artifact`
# lays out `filesystem-digest-*` artifacts with merge-multiple. Writes
