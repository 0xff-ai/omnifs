...
- Report the real frontend teardown wait
- Bound daemon log tail reads
- Reject host frontends without a location
- Support host NFS on Linux
- Normalize command grammar and help
- Unify resource reports and next actions
- Align filesystem lifecycle transcripts
- Unify setup and daemon lifecycle
- Make diagnostics stateful and actionable
- Close command contract gaps
- Reshape setup into boot-and-orient
- Humanize lease errors, settle narration, and the fs empty state
- Render every hint and adopt hints at command-bearing errors
- Honest doctor repair output in every mode
- Single canceled line on prompt cancel
- Uniform launch feedback across drivers
...
- Use online host directly
- Replace frontend lifecycle with named filesystems (#260)
- Preserve events and terminal state
- Focus the TUI on operations and latency
- Move application state behind daemon RPC
- Keep guest and host bindings coexistent
...
- Cache leased namespace replies
- Add atomic mount updates
## [0.2.1] - 2026-06-08
